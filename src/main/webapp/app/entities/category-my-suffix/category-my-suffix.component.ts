...
export class CategoryMySuffixComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.categoryService
      ...
      .subscribe(
        (res: HttpResponse<ICategoryMySuffix[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: ICategoryMySuffix): number {
  ...
  delete(category: ICategoryMySuffix): void {
    const modalRef = this.modalService.open(CategoryMySuffixDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: ICategoryMySuffix[] | null, headers: HttpHeaders, page: number): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.router.navigate(['/category-my-suffix'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
